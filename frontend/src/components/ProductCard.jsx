...
function ProductBadges({ tag, sale, soldOut }) {
  return (
    <div className="absolute top-3 left-3 z-10 flex flex-col gap-1.5">
      {tag && <span className={`badge ${badgeClass(tag)}`}>{tag}</span>}
  ...
  return (
    <>
      <div className="product-card group">
        ...
        <Link to={productHref} className="relative block aspect-[4/5] overflow-hidden bg-[var(--ahps-cream)]">
          <img
            src={p.image}
            alt={p.name}
            loading="lazy"
            className={`${p.hover ? 'img-primary' : 'img-only'} absolute inset-0 w-full h-full object-cover transition-transform duration-700`}
          />
          {p.hover && (
        ...
        <div className="pt-4 pb-3 text-center px-3">
          <h3 className="text-sm font-medium leading-snug line-clamp-2 min-h-[2.5em]">
            <Link to={productHref} className="hover:underline">{p.name}</Link>
          </h3>
